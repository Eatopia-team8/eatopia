# Gemini Instruction Guide (Sparta Convention)
...
### DTO
```java
// Request DTO: 소문자로 시작, 도메인명 + 동작 + Request
ProductCreateRequest
UserSignupRequest

// Response DTO: 소문자로 시작, 도메인명 + Response
ProductResponse
UserResponse
```
...
// 조건부 조회 - WithQuery 추가
public List<ProductResponse> getProductsWithQuery(String keyword, String sortBy);
```
...
// 판매자 화면에 보이는 자신이 등록한 상품 전체 조회
getProducts(Long sellerId)

// 필터 처리가 있는 조회
getOrders(@RequestParam(required=false) Long userId,
...
### Record 사용
* **record + 정적 팩토리 메서드** 조합 사용
* `from` / `of` 사용
* **비즈니스를 나타내는 메서드명이 가장 좋음!**
...
// 비즈니스를 나타내는 메소드명 사용 (from, of 보다 우선)
...
public class OrderCommandServiceImpl implements OrderCommandService {
    ...
    private final ProductQueryServiceImpl productQueryService;  // ❌ 구현체 참조 금지
}
...
* **Swagger**를 사용하여 API 테스트 작성 및 동작 확인
